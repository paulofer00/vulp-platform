...
import { toggleCompanyLike } from "@/app/actions/likes";

export default function FoxLikeButton({ companyId, initialIsLiked }: { companyId: string, initialIsLiked: boolean }) {
  ...
  async function handleToggle() {
    ...
    setIsLiked(newState);
    setIsAnimating(true);
    setTimeout(() => setIsAnimating(false), 300); // Efeito de "pulo"

    // 2. Salva no banco em segundo plano
    await toggleCompanyLike(companyId, !newState); 
  }

  return (
    <button 
        onClick={handleToggle}
        className={`relative group transition-transform duration-300 ease-spring ${isAnimating ? 'scale-125' : 'scale-100'}`}
        title={isLiked ? "Remover dos favoritos" : "Favoritar empresa"}
    >
        {/* SVG DA RAPOSINHA VULP ORIGINAL 🦊 */}
        <svg 
            id="Camada_1" 
